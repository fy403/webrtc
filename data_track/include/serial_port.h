...
public:
    ...
    bool openPort() {
        // 先以 O_NONBLOCK 打开（避免 open() 在 DCD 上阻塞）
        // 然后立即清除  O_NONBLOCK，与 Python pyserial 行为一致
        ...
        // 清除 O_NONBLOCK，恢复阻塞模式（与 pyserial 一致）
        int flags = fcntl(fd, F_GETFL, 0);
        if (flags == -1 || fcntl(fd, F_SETFL, flags & ~O_NONBLOCK) == -1) {
            std::cerr << "清除 O_NONBLOCK 失败: " << strerror(errno) << std::endl;
            close(fd);
            fd = -1;
            return false;
        }
