...
cmd_set() {
    ...
    # -----------------------------
    # Step 5: Video device (may fail if rkaiq is streaming → just warn)
    # -----------------------------
    info "5/5 ${VIDEO_DEV} → NV12 ${WIDTH}x${HEIGHT}"
    if ! v4l2-ctl -d "$VIDEO_DEV" \
        --set-fmt-video=width="$WIDTH",height="$HEIGHT",pixelformat=NV12 \
        2>/dev/null; then
        warn "${VIDEO_DEV} 格式设置失败 (设备可能正被占用，rkaiq 会自动管理格式)"
    fi
    ...
    # Step 6: Apply horizontal banding fix (unless skipped)
